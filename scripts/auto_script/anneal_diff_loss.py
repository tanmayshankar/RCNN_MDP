		...
		for n in range(-w,w+1):
			...
			for u in range(-w,w+1):
				for v in range(-w,w+1):
					loss[w+m,w+n] -= target_belief[previous_pose[0]+u,previous_pose[1]+v] - to_state_belief[current_pose[0]+u,current_pose[1]+v]
...
def master(action_index, time_index):
	...
	# back_prop(action_index)
	simulated_model(action_index)	
	difference_grad(action_index, time_index)
	recurrence()	
...
def input_actions():
	global action, state_counter, action_index, current_pose, previous_pose, time_limit
