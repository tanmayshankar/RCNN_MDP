...
def sum_trans_update(action_index, dummy_index):
	global trans_mat_unknown, to_state_belief, from_state_belief, target_belief, current_pose, previous_pose
	# dummy=copy.deepcopy(trans_mat_unknown)
	dummy = npy.zeros(shape=(transition_space,transition_space))
	# dummy[current_pose[0]-previous_pose[0],current_pose[1]-previous_pose[1]]=1.
	m = action_space[dummy_index,0]
	n = action_space[dummy_index,1]
	# dummy[action_space[dummy_index][0]][action_space[dummy_index][1]]=1.
	dummy[m,n]=1.
	trans_mat_unknown[action_index] +=dummy
	trans_mat_unknown[action_index] /= trans_mat_unknown[action_index].sum()
...
def master(action_index):
	...
	belief_prop(action_index)	
	# back_prop(action_index)
	dummy_ind= simulated_model(action_index)	
	# difference_grad(action_index)
	sum_trans_update(action_index,dummy_ind)
	recurrence()	
