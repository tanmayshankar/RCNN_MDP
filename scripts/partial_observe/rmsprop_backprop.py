...
def rms_backprop_trans(action_index):
	...
	for m in range(-w,w+1):
		for n in range(-w,w+1):
			...
			step_value[w+m,w+n] = learning_rate / math.sqrt(trans_gradients[w+m,w+n]) * trans_grad_temp[w+m,w+n]

			if (trans_mat_unknown[w+m,w+n]-step_value[w+m,w+n]>=0)and(trans_mat_unknown[w+m,w+n]-step_value[w+m,w+n]<1):
				trans_mat_unknown[w+m,w+n]-=step_value[w+m,w+n]
