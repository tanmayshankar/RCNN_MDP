...
def bayes_obs_fusion(t):
	...
	# print "TO BELIEF: ", to_state_belief
	# print "EXT:", ext_to_bel
...
def learn_trans_weighted(action_index, time_index):
	global trans_mat_unknown, transition_space, to_state_belief
	...
	for i in range(-h,h+1):
		for j in range(-h,h+1):
			trans_mat_unknown[action_index,h+i,h+j] += alpha * to_state_belief[observed_state[0]+i,observed_state[1]+j]
			# trans_mat_unknown[action_index,h+i,h+j] += target_belief[observed_state[0]+i,observed_state[1]+j]
			# trans_mat_unknown[action_index,h+i,h+j] += from_state_belief[observed_state[0]+i,observed_state[1]+j]
...
def master_weighted_counting(action_index,time_index):
